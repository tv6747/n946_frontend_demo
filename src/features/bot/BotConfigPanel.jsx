...
export function BotConfigPanel({ bot, isCreating, associatedFiles, folderFiles, selectedFolderName, users, onUpdateBot, onCreateBot, onRemoveFile, onDeleteBot, onOpenLLMSettings }) {
  ...
  const handleUserToggle = (userId) => {
    ...
    const newUsers = currentUsers.includes(userId)
      ...
      : [...currentUsers, userId];
    handleChange({ accessibleUsers: newUsers });
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                  ...
                  <div className="min-h-[120px]">
                      {activeTab === 'welcome' ? (
                          <textarea 
                            ...
                            rows={4}
                            className="w-full px-4 py-3 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-400 resize-none transition-all"
                        />
                      ) : (
          ...
          {/* 4. 第三區塊：權限設定 Permissions */}
          <section className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 mb-6">
              <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-4 border-b border-slate-100 pb-2">使用權限設定</h3>
              ...
              <div className="mb-4 max-w-md relative">
                   ...
                   <input 
                        ...
                        placeholder="搜尋人員或部門..." 
                        className="w-full pl-9 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-400 transition-all"
                   />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-[200px] overflow-y-auto custom-scrollbar">
                  {filteredUsers.map(user => {
                     const isSelected = (formData.accessibleUsers || []).includes(user.id);
                     return (
                         <div 
                           key={user.id} 
                         ...
                         >
                             ...
                             <div className="flex items-center gap-2 min-w-0">
                                 ...
                                 <span className={`text-sm truncate ${isSelected ? 'font-medium text-blue-800' : 'text-slate-700'}`}>
                                     {user.name}
                                 </span>
                             </div>
                         </div>
