...
- `ProperNounPanel`: 專有名詞與定義管理 (已移除函釋說明提示)。
  - **NEW**: 將原本的「模型」細分為「模型類型」和「模型」。
    - **模型類型**: 包含 Type 0 (適用所有場合)、Type 1 (檔案辨識專用)、Type 2 (即時辨識專用)。
    - **模型**: 針對特定的AI模型 (例如 NLMA-Speech-V1)。
    - 兩者皆可以作為列表的下拉篩選器，並在新增或編輯詞彙時進行設定。
  - 機率值調整：支援 0 ~ 1 區間的滑桿，結合輸入框精準設定。
- `SynonymPanel`: (原「近似詞語料庫」更名為「勘誤表管理」) 近似詞與標準詞對應管理。
  - 將輸入欄位變更為「勘誤前詞彙」與「勘誤後詞彙」。
  - 修正時原「勘誤前詞彙」為唯讀鎖定狀態。
  - **NEW**: 支援「適用模型」設定。包含頂部列表的模型篩選器、列表顯示對應模型，以及新增編輯彈窗時可指派適用的模型。
...
- **提示詞優化** (`PROMPT_OPT`): `PromptOptimizerInterface` (含內建載入提示詞 Modal、存檔 Modal)
...
| **載入提示詞 Modal** | 提示詞優化「載入提示詞」按鈕 | 兩步驟選擇器：先選提示詞再選版本 (內嵌於 PromptOptimizerInterface) |
| **存檔 Modal** | 提示詞優化儲存圖示 | 另存新增或建立新版本/覆蓋 (內嵌於 PromptOptimizerInterface) |
